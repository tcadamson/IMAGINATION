...
PRESETS_PATH: typing.Final = USER_DIRECTORY / "presets.json"


def load_presets() -> dict[str, str]:
    """Load and parse presets.json into a preset dict."""
    try:
        with PRESETS_PATH.open(encoding="utf-8") as fp:
            data = json.load(fp)
    except FileNotFoundError:
        return {}
    except json.JSONDecodeError as exception:
        raise RuntimeError(f"Malformed JSON at {PRESETS_PATH}") from exception
